...
public class HomePage  {
    ...
    public void Addcontent_SF(WebBrowser app) throws Exception {
    	try {
    		app.getActions().inputText(By.xpath("(//*[@type='text'])[2]"), "demotest");
    		System.out.println("Entered Account name successfully.");
    	}catch(Exception e) {
    		System.out.println("Unable to enter Account name : "+e.getMessage());
    		except = "true";
    		Assert.assertEquals(except,true,"Unable to enter Account name, Reason: "+e.getMessage());	
    	}
    	
    	try {
    		app.getActions().click(By.xpath("(//*[@value='Save'])[2]"), TimeoutType.DEFAULT);
    		System.out.println("Clicked save button Successfully.");
    	}catch(Exception e) {
    		System.out.println("Unable to click save button : "+e.getMessage());
    		except = "true";
    		Assert.assertEquals(except,true,"Unable to click save button, Reason: "+e.getMessage());	
    	}
    	try {
    		app.getActions().verifyElementVisible(By.xpath("//*[text()='Customize Page']"), TimeoutType.DEFAULT);
    		System.out.println("page added Successfully.");
    	}catch(Exception e) {
    		System.out.println("Unable to add page : "+e.getMessage());
    		except = "true";
    		Assert.assertEquals(except,true,"Unable to add page, Reason: "+e.getMessage());	
    	}
